...
def mng2_find_queue(queue_name):
    return collection.find_one({"name": queue_name})


def mng2_get_users(queue_name):
    queue = mng2_find_queue(queue_name)
    if queue:
        users = []
...
def mng2_join_queue(queue_name, user):
    queue = mng2_find_queue(queue_name)
    if not queue:
        return 'Queue {} not exist!'.format(queue_name)
...
def mng2_leave_queue(queue_name, user):
    queue = mng2_find_queue(queue_name)
    if not queue:
        return 'Queue {} not exist!'.format(queue_name)
...
def mng2_rejoin_queue(queue_name, user):
    # counter = db[queue_name].count_documents({})
    # if counter == 0:
    #     return "Queue {} empty!You need join first!".format(queue_name)
    # if counter == 1 and db[queue_name].find_one({"_id": user.id}):
    ...
    queue = collection.find_one({"name": queue_name, "users.1": {"$exists": "false"}})
    if queue is not None:
        return "Person {} can't re-join because he is the only one in the Queue {}!".format(user.name, queue_name)

    msg = mng2_leave_queue(queue_name, user) + '\n' + mng2_join_queue(queue_name, user)
    return msg
